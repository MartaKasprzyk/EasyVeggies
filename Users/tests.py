...
def test_register_user_view_post_passwords_different():
    client = Client()
    url = reverse('registration')
    data = {'username': 'test_user', 'password': 'abcd', 'password_repeat': 'dcba'}
...
def test_register_user_view_post_success():
    client = Client()
    url = reverse('registration')
    data = {'username': 'test_user', 'password': 'abcd', 'password_repeat': 'abcd'}
...
def test_register_user_view_post_username_exists(user):
    client = Client()
    url = reverse('registration')
    data = {'username': 'test', 'password': 'abcd', 'password_repeat': 'abcd'}
    response = client.post(url, data)
    assert response.status_code == 200
